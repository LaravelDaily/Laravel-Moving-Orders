    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.movings.update", [$moving->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label class="required" for="user_id">{{ trans('cruds.moving.fields.user') }}</label>
                <select class="form-control select2 {{ $errors->has('user') ? 'is-invalid' : '' }}" name="user_id" id="user_id" required>
                    @foreach($users as $id => $user)
                        <option value="{{ $id }}" {{ (old('user_id') ? old('user_id') : $moving->user->id ?? '') == $id ? 'selected' : '' }}>{{ $user }}</option>
                    @endforeach
                </select>
                @if($errors->has('user'))
                ...
                @endif
                <span class="help-block">{{ trans('cruds.moving.fields.user_helper') }}</span>
            </div>
            <div class="form-group">
